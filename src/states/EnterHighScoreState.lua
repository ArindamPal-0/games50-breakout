...
function EnterHighScoreState:enter(params)
    ...
    -- print('EnterHighScoreState.lua:enter')
    -- print(self.highScores[1].name)
    -- print(self.highScores[1].score)
    -- print(self.highScores[10].name)
    -- print(self.highScores[10].score)
    -- print('current score: ' .. tostring(self.score))
    -- print('scoreIndex: ' .. tostring(self.scoreIndex))

    -- resetting chars table
    for i = 1, 3 do
        chars[i] = 65
    end
end
...
function EnterHighScoreState:update(dt)
    ...
    if (love.keyboard.wasPressed('left') or love.keyboard.wasPressed('a')) and highlightedChar > 1 then
        highlightedChar = highlightedChar - 1
        gSounds['select']:play()
    elseif (love.keyboard.wasPressed('right') or love.keyboard.wasPressed('d')) and highlightedChar < 3 then
        highlightedChar = highlightedChar + 1
        gSounds['select']:play()
    end

    -- scroll through characters in individual slot
...
function EnterHighScoreState:render()
    ...
    -- render all three characters of the same name
